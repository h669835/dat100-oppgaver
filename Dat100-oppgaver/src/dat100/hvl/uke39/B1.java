...
public class B1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(antall_ledig(aud));
		System.out.println(prosent_opptatt(aud));
	}

	public static int antall_ledig(boolean[][] aud) {
		int antall = 0;
	...
	public static int prosent_opptatt(boolean[][] aud) {
		int antall_ledige = 0;
		int antall_total = 0;
		for (int i = 0; i < aud.length; i++) {
			for (int j = 0; j < aud[i].length; j++) {
		...
		return (int) (((double) antall_total - (double) antall_ledige) / (double) antall_ledige * 100);
	}
}
